...
function DrawBars (cr,start_x,start_y,bar_width,bar_height,corenum,r,g,b)
    ...
    value = tonumber(conky_parse(string.format("${cpu cpu%s}",corenum)))
    max_value=100
...
--Check that Conky has been running for at least 5s
    ...
    DrawLine(cr,51,0,294,0,8)
    DrawLine(cr,343,0,0,25,4)
    DrawLine(cr,51,75,220,0,4)

    --draw network lines
    DrawLine(cr,398,155,0,23,4)
    DrawLine(cr,51,155,349,0,4)
    --draw cpu temp bars
    ...
    --draw cpu temp lines
    DrawLine(cr,51,320,299,0,4)
    DrawLine(cr,348,318,0,26,4)
    --draw mem lines
    DrawLine(cr,51,585,94,0,4)
